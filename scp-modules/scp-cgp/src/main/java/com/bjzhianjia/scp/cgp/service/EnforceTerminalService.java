...
public class EnforceTerminalService {
    ...
    private void queryAssist(List<EnforceTerminal> list) {
        ...
        // List<String> uniqueDeparts =
        // list.stream().map((o)->o.getRetrievalDepartment()).distinct().collect(Collectors.toList());

        /*
         * 数据库中，领用人与领用部门都是可空，所以如果当这两个字段为空时，
         * uniqueDeparts和uniqueUsers字符串中将带有null字样
        ...
        // list.stream().map((o)->o.getRetrievalUser()).distinct().collect(Collectors.toList());
        ...
        if (!departs.isEmpty() || !users.isEmpty()) {
            for (EnforceTerminal tmpTerminal : list) {
                ...
                if (BeanUtil.isNotEmpty(jsonUserObj)) {
                    // 终端领用人有可能为空，造成解析出的jsonUserObj为null，需要进行非空判断
                    userJobjForReturn.put("id", jsonUserObj.getString("id"));
                    userJobjForReturn.put("name", jsonUserObj.getString("name"));
                } else {
                    userJobjForReturn.put("id", "");
                    userJobjForReturn.put("name", "");
                }
